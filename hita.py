...
def main():
    ...
    parser.add_argument('--save_dir', default=f'./saved_models/hita/', help='model output directory')
...
def train(args):
    ...
    grouped_parameters = [
        {'params': [p for n, p in model.encoder.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 0.001, 'lr': args.encoder_lr},
        {'params': [p for n, p in model.encoder.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.001, 'lr': args.encoder_lr},
        {'params': [p for n, p in model.decoder.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': 0.001, 'lr': args.encoder_lr},
        {'params': [p for n, p in model.decoder.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.001, 'lr': args.encoder_lr},
    ]
    ...
    for epoch_id in range(args.n_epochs):
        ...
        print('| step {:5} | train_acc {:7.4f} | dev_acc {:7.4f} | test_acc {:7.4f} | loss {:7.4f} '.format(global_step, train_acc, dev_acc, test_acc,
                                                                                        tl))
        print('| step {:5} | train_precision {:7.4f} | dev_precision {:7.4f} | test_precision {:7.4f} | loss {:7.4f} '.format(global_step,
                                                                                                            tr_precision,
                                                                                                            d_precision,
                                                                                                            t_precision,
                                                                                                            tl))
        print('| step {:5} | train_recall {:7.4f} | dev_recall {:7.4f} | test_recall {:7.4f} | loss {:7.4f} '.format(global_step,
                                                                                                            tr_recall,
                                                                                                            d_recall,
                                                                                                            t_recall,
                                                                                                            tl))
    ...
    print('best dev acc: {:.4f} (at epoch {})'.format(best_dev_auc, best_dev_epoch))
    print('final test acc: {:.4f}'.format(final_test_auc))
    print()
