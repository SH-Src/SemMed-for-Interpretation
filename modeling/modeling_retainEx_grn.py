...
from modeling.retainEx import RETAIN_EX
...
class LMGraphRelationNet(nn.Module):
    def __init__(self, model_name, k, n_type, n_basis, n_layer, diag_decompose,
        ...
        self.encoder = RETAIN_EX(**encoder_config)
        #self.decoder = MLP(256, fc_dim, 2, n_fc_layer, p_fc, layer_norm=True)
        self.decoder = GraphRelationNet(k, n_type, n_basis, n_layer, self.encoder.hidden_size, diag_decompose,
                                        n_concept, n_relation, concept_dim, concept_in_dim, n_attention_head,
                                        fc_dim, n_fc_layer, att_dim, att_layer_num, p_emb, p_gnn, p_fc,
                                        pretrained_concept_emb=pretrained_concept_emb, freeze_ent_emb=freeze_ent_emb,
...
class LMGraphRelationNetDataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
        ...
        self.train_qids, self.train_labels, *self.train_encoder_data = load_retainex_input(train_statement_path, max_seq_length, max_num_pervisit)
        self.dev_qids, self.dev_labels, *self.dev_encoder_data = load_retainex_input(dev_statement_path, max_seq_length, max_num_pervisit)
        ...
        *self.dev_decoder_data, self.dev_adj_data, n_rel = load_adj_data(dev_adj_path, max_node_num, num_choice, emb_pk_path=dev_embs_path if use_contextualized else None)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, *self.test_encoder_data = load_retainex_input(test_statement_path, max_seq_length, max_num_pervisit)
